...
class UserController(private val system: SpotifyService) {
    ...
    fun getUserById(ctx: Context) {
        ...
        try{
            ...
            ctx.status(404).json(ErrorResponse("result : Not found user with id $id"))
        }
    }
    ...
    fun addPlaylist(ctx: Context) {
        ...
        val body = ctx.bodyValidator<PlaylistDraftResponse>()
            ...
            .get()
        try{
            val playlist = system.addPlaylist(id, PlayListDraft(body.name, body.description, body.image, body.songs))
            ...
            ctx.status(404).json(ErrorResponse("result : Not found playlist with id $id"))
        } catch (e: UserException ){
            ctx.status(404).json(ErrorResponse("result : Not found user with id $id"))
        }
    }
